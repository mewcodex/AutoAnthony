...
public sealed class ComponentAssemblyGenerator
{
    ...
    public GeneratedCard Generate(GeneratedRarity rarity)
    {
        ...
        for (var shellAttempt = 0; shellAttempt < ShellRerollLimit; shellAttempt++)
        {
            ...
            if (TryGenerate(rarity, lastShell, ref duplicateFailures, raiseAggressiveEffectFloor,
                    softenAggressiveNegative) is { } generated)
                return generated;
        }
        ...
        for (var fallbackAttempt = 0; fallbackAttempt < 256; fallbackAttempt++)
        {
            var fallback = GenerateEmergencyFallback(rarity, lastShell!, fallbackAttempt,
                raiseAggressiveEffectFloor);
            if (TryFinalizeUniqueCard(fallback, ref duplicateFailures, out var finalized)) return finalized;
        }
        ...
        ref int duplicateFailures, bool raiseAggressiveEffectFloor, bool softenAggressiveNegative)
    {
        ...
        var starCost = NumericGenerationTuning.ApplyBasicStarCostFloor(
            NumericGenerationTuning.ApplyStarCostAdjustment(
                _random, nativeStarCost, _unlockComponentRoles), rarity);
        var hasStarCostX = usesSharedResourceShell && shell.HasStarCostX
        ...
        if (TryFinalizeUniqueCard(card, ref duplicateFailures, out var finalized)) return finalized;
        }
        return null;
    }
        ...
        GeneratedCharacter character = GeneratedCharacter.Ironclad, double downsideFlatValue = 0d)
    {
        ...
        var (lowerPercent, upperPercent) = rarity switch
        {
            ...
            GeneratedRarity.Common => (0.81d, 1.15d),
            GeneratedRarity.Uncommon => (0.85d, 1.18d),
...
public static class CardEffectRules
{
    ...
    private static bool IsGenericDependencyPayoff(GeneratorOperation prefix, ComponentAtom payoff)
    {
        if (payoff.Scope is not (OperationScope.SingleEnemyOnly or OperationScope.NonTargeted
            or OperationScope.Independent)) return false;
...
/// <summary>
/// Native v111 single-player rarity. Kept in the standalone generator so type, effect-count, keyword, and component
/// frequencies can be measured by source rarity without referencing game assemblies.
/// </summary>
internal static class OriginalCardRarities
{
    private static readonly IReadOnlyDictionary<string, GeneratedRarity> Values = Build();

    public static GeneratedRarity Get(string id) => Values.TryGetValue(id, out var rarity)
        ? rarity
        : throw new InvalidOperationException($"原卡稀有度表缺少 {id}。");

    private static IReadOnlyDictionary<string, GeneratedRarity> Build()
    {
        var result = new Dictionary<string, GeneratedRarity>(StringComparer.Ordinal);
        Add(result, GeneratedRarity.Basic, "Bash|DefendIronclad|StrikeIronclad|DefendSilent|Neutralize|StrikeSilent|Survivor");
        Add(result, GeneratedRarity.Common, "Anger|Armaments|BloodWall|BodySlam|Breakthrough|Cinder|Havoc|Headbutt|IronWave|MoltenFist|PerfectedStrike|PommelStrike|SetupStrike|ShrugItOff|SwordBoomerang|Taunt|Thunderclap|Tremble|TrueGrit|TwinStrike|Anticipate|Backflip|BladeDance|CloakAndDagger|DaggerSpray|DaggerThrow|DeadlyPoison|Deflect|DodgeAndRoll|FlickFlack|LeadingStrike|PiercingWail|PoisonedStab|Predator|Prepared|Ricochet|Slice|Snakebite|SuckerPunch|Untouchable");
        Add(result, GeneratedRarity.Uncommon, "AshenStrike|BattleTrance|Bloodletting|Bludgeon|Bully|BurningPact|Colossus|Cruelty|Dismantle|DrumOfBattle|EvilEye|ExpectAFight|FeelNoPain|FightMe|FlameBarrier|ForgottenRitual|Hemokinesis|HowlFromBeyond|InfernalBlade|Inferno|Inflame|Juggling|Pillage|Rage|Rampage|Rupture|SecondWind|Spite|Stampede|Stomp|StoneArmor|Unrelenting|Uppercut|Vicious|Whirlwind|Accelerant|Accuracy|Acrobatics|Backstab|Blur|BouncingFlask|BubbleBubble|CalculatedGamble|Dash|EchoingSlash|EscapePlan|Expertise|Expose|Finisher|Flechettes|Sidestep|Footwork|HandTrick|Haze|HiddenDaggers|InfiniteBlades|LegSweep|MementoMori|Mirage|NoxiousFumes|PhantomBlades|Pinpoint|Pounce|PreciseCut|Reflex|Skewer|Speedster|Strangle|Tactician|UpMySleeve");
        Add(result, GeneratedRarity.Rare, "Aggression|Barricade|Brand|Cascade|Conflagration|CrimsonMantle|DarkEmbrace|DemonForm|Dominate|Feed|FiendFire|Hellraiser|Impervious|Juggernaut|Mangle|NotYet|Offering|OneTwoPunch|PactsEnd|PrimalForce|Pyre|Stoke|TearAsunder|Thrash|Unmovable|Abrasive|Adrenaline|Afterimage|Assassinate|BladeOfInk|BulletTime|Burst|CorrosiveWave|Envenom|FanOfKnives|GrandFinale|KnifeTrap|Malaise|MasterPlanner|Murder|Nightmare|Outbreak|SerpentForm|ShadowStep|Shadowmeld|StormOfSteel|TheHunt|ToolsOfTheTrade|Tracking|WellLaidPlans");
        Add(result, GeneratedRarity.Ancient, "Break|Corruption|Suppress|WraithForm");
        return result;
    }

    private static void Add(IDictionary<string, GeneratedRarity> target, GeneratedRarity rarity, string ids)
    {
        foreach (var id in ids.Split('|'))
            target.Add(id, rarity);
    }
}
...
/// <summary>Loads 85 native component sequences from the reviewed Ironclad decomposition embedded in the assembly.</summary>
public sealed class IroncladComponentCatalog : IComponentCatalog
{
    private static readonly Lazy<IroncladComponentCatalog> _instance = new(Load);
    public static IroncladComponentCatalog Instance => _instance.Value;
    public GeneratedCharacter Character => GeneratedCharacter.Ironclad;
    ...
    private IroncladComponentCatalog(IReadOnlyList<IroncladCardRecipe> recipes)
    {
        Recipes = recipes;
        ...
        AtomKeys = allAtoms.Select(atom => atom.Key).ToHashSet();
        AtomCounts = allAtoms.GroupBy(atom => atom.Key).ToDictionary(group => group.Key, group => group.Count());
    ...
    private static IroncladComponentCatalog Load()
    {
        var assembly = Assembly.GetExecutingAssembly();
        var resource = assembly.GetManifestResourceNames().Single(name => name.EndsWith("ironclad_unit_operations.md", StringComparison.Ordinal));
        using var stream = assembly.GetManifestResourceStream(resource) ?? throw new InvalidOperationException("缺少嵌入的战士组件目录。");
        ...
        var recipes = reader.ReadToEnd().Split('\n')
            ...
            .ToArray();
        if (recipes.Length != 85)
            throw new InvalidOperationException($"战士组件目录应为 85 张卡，实际读取到 {recipes.Length} 张。");
        return new IroncladComponentCatalog(CatalogRuntimeSpecRegistry.Attach(GeneratedCharacter.Ironclad,
            recipes));
    }

    internal static IroncladCardRecipe? ParseRow(string line)
    {
        ...
        var chineseTitle = cells[1][..idMatch.Index].Trim();
        ...
        var type = meta[1] switch { "攻击" => GeneratedCardType.Attack, "技能" => GeneratedCardType.Skill, "能力" => GeneratedCardType.Power, _ => throw new InvalidOperationException(meta[1]) };
        var target = meta[2] == "单敌" ? TargetMode.SingleEnemy : TargetMode.Other;
        var tags = ParseTags(cells[4]);
        ...
        return new IroncladCardRecipe(
            ...
            type,
            target,
            OriginalCardRarities.Get(id),
            tags,
            parsedAtoms.Select(item => item.Atom).ToArray(),
            parsedAtoms.Select(item => item.TriggerOwner).ToArray());
    }

    private static IReadOnlyList<CardTag> ParseTags(string cell)
    {
        var tags = new List<CardTag>();
        if (cell.Contains("Strike", StringComparison.Ordinal)) tags.Add(CardTag.Strike);
        if (cell.Contains("Defend", StringComparison.Ordinal)) tags.Add(CardTag.Defend);
        if (cell.Contains("Exhaust", StringComparison.Ordinal)) tags.Add(CardTag.Exhaust);
        if (cell.Contains("Innate", StringComparison.Ordinal)) tags.Add(CardTag.Innate);
        if (cell.Contains("Retain", StringComparison.Ordinal)) tags.Add(CardTag.Retain);
        if (cell.Contains("Sly", StringComparison.Ordinal)) tags.Add(CardTag.Sly);
        return tags;
    }

    private static IReadOnlyList<(ComponentAtom Atom, int TriggerOwner)> ParseAtoms(string cell)
    {
        var result = new List<(ComponentAtom Atom, int TriggerOwner)>();
        var parts = SplitTopLevel(cell.Replace('`', ' ').Trim(), '；');
        foreach (var part in parts)
        {
            var arrow = part.IndexOf("=>", StringComparison.Ordinal);
            if (arrow >= 0)
            {
                var leftComponents = SplitComponents(part[..arrow]).ToArray();
                foreach (var component in leftComponents)
                    result.Add((Atom(component), -1));
                var triggerIndex = result.Count - 1;
                foreach (var effect in SplitComponents(part[(arrow + 2)..]))
                    result.Add((Atom(effect), triggerIndex));
            }
            else
            {
                foreach (var component in SplitComponents(part))
                    result.Add((Atom(component), -1));
            }
        }
        return result;
    ...
    private static IEnumerable<string> SplitTopLevel(string text, char delimiter)
    {
        ...
        for (var index = 0; index < text.Length; index++)
        {
            ...
            else if (text[index] is ')' or '）') depth = Math.Max(0, depth - 1);
            else if (text[index] == delimiter && depth == 0)
            {
    ...
    private static IEnumerable<string> SplitComponents(string part) => Regex.Split(part.Trim(), @"\s+\+\s+(?=[A-Z]:)").Where(component => !string.IsNullOrWhiteSpace(component));

    private static ComponentAtom Atom(string raw)
    {
        var text = raw.Trim();
        var template = NormalizeTemplate(Regex.Match(text, @"^[A-Z]:[^\s(]+").Value);
        // Give execution-sensitive parameterized effects stable semantic IDs. Runtime text matching remains
        // only as compatibility for snapshots generated before these IDs existed.
        if (text.StartsWith("N:Self(S, count(target.V)", StringComparison.Ordinal))
            template = "N:StrengthPerTargetVulnerable";
        else if (text.StartsWith("M:base + 3 × count(消耗牌堆全部牌)", StringComparison.Ordinal))
            template = "M:DamagePerExhaustCard";
        else if (text.StartsWith("A:when(打出 Skill)", StringComparison.Ordinal))
            template = "A:whenSkillPlayed";
        else if (text.StartsWith("N:Create(random card from own pool, 手牌, 1)", StringComparison.Ordinal))
            template = "N:CreateCurrentCharacterCardInHand";
        var scope = template.StartsWith("T:", StringComparison.Ordinal) ? OperationScope.SingleEnemyOnly
            : template.StartsWith("N:", StringComparison.Ordinal) ? OperationScope.NonTargeted
            : template.StartsWith("M:", StringComparison.Ordinal) ? OperationScope.Modifier
            : template.StartsWith("A:rule", StringComparison.Ordinal) ? OperationScope.AbilityRule
            : template.StartsWith("A:", StringComparison.Ordinal) ? OperationScope.AbilityTrigger
            : template.StartsWith("C:", StringComparison.Ordinal) ? OperationScope.ConditionalTrigger
            : OperationScope.Independent;
        // Dominate-style self effects may read target values and therefore still depend on a selected enemy.
        var requiresSingleTarget = text.Contains("target.", StringComparison.OrdinalIgnoreCase)
            || text.Contains("该敌人", StringComparison.Ordinal)
            || text.Contains("敌人身上", StringComparison.Ordinal)
            || template is "I:PlayExhaustedShivsAtTarget";
        return new ComponentAtom(
            string.IsNullOrEmpty(template) ? "I_RAW" : template,
            scope,
            ToChinese(text),
            requiresSingleTarget,
            InferCardReference(text, template));
    }

    private static string NormalizeTemplate(string template)
    {
        if (!Regex.IsMatch(template, @"[一-龥]")) return template;
        return template switch
        {
            var value when value.StartsWith("I:本场战斗中此卡的基础伤害", StringComparison.Ordinal) => "I:IncreaseDamageThisCombat",
            "I:本回合不能再抽牌" => "I:PreventDrawThisTurn",
            "I:本回合获得S" => "I:GainTemporaryStrength",
            "I:变形" => "I:TransformCards",
            "I:抽牌至条件" => "I:DrawUntilCondition",
            var value when value.StartsWith("I:从手牌随机自动打出", StringComparison.Ordinal) => "I:AutoPlayRandomAttackFromHand",
            var value when value.StartsWith("I:打出抽牌堆顶部X张牌", StringComparison.Ordinal) => "I:PlayTopXCards",
            "I:打出抽牌堆顶牌，并强制消耗该牌" => "I:PlayTopCardAndExhaust",
            "I:打出此牌" => "I:PlayThisCard",
            "I:对每名敌人Apply" => "I:ApplyToAllEnemies",
            "I:对一名随机敌人打出该牌" => "I:PlayAtRandomEnemy",
            "I:额外打出一次该攻击" => "I:ReplayAttack",
            "I:费用变为0" => "I:SetCostZero",
            "I:费用规则" => "I:CostRule",
            "I:将该攻击牌的当前伤害永久加到此卡基础伤害" => "I:AddExhaustedAttackDamage",
            var value when value.StartsWith("I:连续抽牌，直到抽到非攻击牌", StringComparison.Ordinal) => "I:DrawUntilNonAttack",
            "I:随机消耗手牌1张Attack" => "I:ExhaustRandomAttack",
            var value when value.StartsWith("I:永久最大生命", StringComparison.Ordinal) => "I:GainMaxHp",
            "I:自动打出" => "I:AutoPlay",
            "N:反伤攻击者D" => "N:RetaliateDamage",
            _ => throw new InvalidOperationException($"缺少英文 operation ID 映射：{template}")
        };
    }

    private static CardReferenceRequirement InferCardReference(string text, string template)
    {
        // Havoc-style operations create a local top-deck card and immediately operate on it; no external slot is consumed.
        if (text.Contains("打出抽牌堆顶牌", StringComparison.Ordinal)) return CardReferenceRequirement.None;
        // Pummel-style resolution selects its own random Attack; subsequent references use that local object, not a choice slot.
        if (text.Contains("随机消耗手牌1张Attack", StringComparison.Ordinal)
            || text.Contains("将该攻击牌的当前伤害永久加到此卡基础伤害", StringComparison.Ordinal)) return CardReferenceRequirement.None;
        // In these DSL forms the adjacent trigger event supplies the referenced card; no extra choice slot is needed.
        if (text.StartsWith("N:Exhaust(该技能)", StringComparison.Ordinal)
            || text.StartsWith("N:Exhaust(该牌)", StringComparison.Ordinal)
            || text.StartsWith("N:Create(copy(该攻击)", StringComparison.Ordinal)
            || text.StartsWith("I:对一名随机敌人打出该牌", StringComparison.Ordinal)
            || text.StartsWith("I:额外打出一次该攻击", StringComparison.Ordinal)
            || text.StartsWith("I:费用变为0", StringComparison.Ordinal)) return CardReferenceRequirement.None;
        if (text.Contains("消耗那张技能牌", StringComparison.Ordinal)) return CardReferenceRequirement.None;
        if (text.Contains("消耗那张非攻击牌", StringComparison.Ordinal)
            || text.Contains("将那张攻击牌", StringComparison.Ordinal)
            || text.Contains("对一名随机敌人打出该牌", StringComparison.Ordinal)
            || text.Contains("将该攻击牌额外打出", StringComparison.Ordinal)
            || template.Contains("UpgradeThatCard", StringComparison.Ordinal)) return CardReferenceRequirement.None;
        if (text.Contains("exhausted(card)", StringComparison.Ordinal) || text.Contains("card在消耗堆", StringComparison.Ordinal)) return CardReferenceRequirement.ThisCard;
        if (text.Contains("此牌", StringComparison.Ordinal) || text.Contains("这张牌", StringComparison.Ordinal)) return CardReferenceRequirement.ThisCard;
        if (text.Contains("该攻击", StringComparison.Ordinal) || text.Contains("自动打出该牌", StringComparison.Ordinal)) return CardReferenceRequirement.HandAttack;
        if (text.Contains("该牌", StringComparison.Ordinal) || text.Contains("该技能", StringComparison.Ordinal) || text.Contains("那张牌", StringComparison.Ordinal) || template.Contains("UpgradeThatCard", StringComparison.Ordinal)) return CardReferenceRequirement.HandCard;
        return CardReferenceRequirement.None;
    }

    private static string ToChinese(string text)
    {
        text = text.Trim();
        var areaDamage = Regex.Match(text, @"^N:AllD\((\d+), hits=(.+)\)");
        if (areaDamage.Success)
            return Finish($"对所有敌人造成{areaDamage.Groups[1].Value}点伤害{HitSuffix(areaDamage.Groups[2].Value)}");
        var randomDamage = Regex.Match(text, @"^N:RandomD\((\d+), hits=(.+)\)");
        if (randomDamage.Success)
            return Finish($"随机对敌人造成{randomDamage.Groups[1].Value}点伤害{HitSuffix(randomDamage.Groups[2].Value)}");
        ...
        var rendered =
            Match(@"^T:D\((\d+)\)", "造成{0}点伤害") ??
            Match(@"^N:B\((\d+)\)", "获得{0}点格挡") ??
            Match(@"^N:Draw\((\d+)\)", "抽{0}张牌") ??
            Match(@"^N:E\((\d+)\)", "获得{0}点能量") ??
            Match(@"^N:HP-\((\d+)\)", "失去{0}点生命") ??
            Match(@"^N:Heal\((\d+)\)", "回复{0}点生命") ??
            Match(@"^T:Apply\(V,(\d+)\)", "给予{0}层易伤") ??
            Match(@"^T:Apply\(W,(\d+)\)", "给予{0}层虚弱") ??
            Match(@"^T:Poison\((\d+)\)", "给予{0}层中毒") ??
            Match(@"^N:Dex\((\d+)\)", "获得{0}点敏捷") ??
            Match(@"^N:Thorns\((\d+)\)", "获得{0}点荆棘") ??
            Match(@"^N:TempDex\((\d+)\)", "本回合获得{0}点敏捷") ??
            Match(@"^N:Intangible\((\d+)\)", "获得{0}层无实体") ??
            Match(@"^N:LoseDex\((\d+)\)", "失去{0}点敏捷") ??
            Match(@"^N:Discard\((\d+)\)", "丢弃{0}张牌") ??
            Match(@"^N:CreateShiv\((\d+)\)", "将{0}张小刀加入手牌") ??
            Match(@"^N:CreateInkShiv\((\d+)\)", "将{0}张墨影小刀加入手牌") ??
            Match(@"^N:KeepBlockNextTurn\((\d+)\)", "你的下一回合开始时格挡不会消失") ??
            Match(@"^N:NextTurnBlock\((\d+)\)", "在下个回合，获得{0}点格挡") ??
            Match(@"^N:NextTurnEnergy\((\d+)\)", "在下个回合，获得{0}点能量") ??
            Match(@"^N:NextTurnDraw\((\d+)\)", "在下个回合，抽{0}张牌") ??
            Match(@"^N:AllPoison\((\d+)\)", "给予所有敌人{0}层中毒") ??
            Match(@"^N:AllWeak\((\d+)\)", "给予所有敌人{0}层虚弱") ??
            Match(@"^N:AllTempStrengthLoss\((\d+)\)", "所有敌人在本回合失去{0}点力量") ??
            Match(@"^T:Strangle\((\d+)\)", "本回合你每打出一张牌，该敌人失去{0}点生命") ??
            Match(@"^N:RandomPoison\((\d+), hits=(\d+)\)", "随机给予敌人{0}层中毒{1}次") ??
            (text == "T:XStrengthLoss" ? "敌人失去X点力量" : null) ??
            (text == "T:XWeak" ? "给予X层虚弱" : null) ??
            Match(@"^T:DX\((\d+)\)", "造成{0}点伤害X次") ??
            (text == "N:DiscardAll" ? "丢弃所有手牌" : null) ??
            (text == "N:BlockEqualAllPoison" ? "获得等同于所有敌人中毒层数总和的格挡" : null) ??
            Match(@"^N:Self\(S,(\d+)\)", "获得{0}点力量") ??
            Match(@"^N:Self\(覆甲,(\d+)\)", "获得{0}层覆甲") ??
            Match(@"^N:Exhaust\(全部, 手牌\)", "消耗所有手牌") ??
            Match(@"^N:Exhaust\(全部非Attack, 手牌\)", "消耗手牌中的所有非攻击牌") ??
            Match(@"^N:Exhaust\(选择1张, 手\)", "消耗手牌中的一张牌") ??
            Match(@"^N:Exhaust\(随机1张, 手牌\)", "随机消耗手牌中的一张牌") ??
            Match(@"^N:Exhaust\(该牌\)", "消耗那张非攻击牌") ??
            Match(@"^N:Create\(copy\(该攻击\), 手牌, 1\)", "将那张攻击牌的一张复制加入手牌") ??
            Match(@"^N:Create\(copy\(card\), 弃牌堆, 1\)", "将此牌的一张复制加入弃牌堆") ??
            (text.StartsWith("M:base + 3 × count(消耗牌堆全部牌)", StringComparison.Ordinal) ? "消耗牌堆每有一张牌，这张牌额外造成3点伤害" : null) ??
            (text.StartsWith("M:base + 2 × count(target.V)", StringComparison.Ordinal) ? "该敌人每有一层易伤，这张牌额外造成2点伤害" : null) ??
            (text.StartsWith("M:base + 5 × max(0, self.S)", StringComparison.Ordinal) ? "你每有1点力量，这张牌额外获得5点格挡" : null) ??
            (text.StartsWith("M:base + 2 × count(本场战斗所有区域中 Tag:Strike 卡)", StringComparison.Ordinal) ? "本场战斗中每有一张名称含“打击”的牌，这张牌额外造成2点伤害" : null) ??
            (text.StartsWith("M:value = state(当前格挡)", StringComparison.Ordinal) ? "这张牌造成等同于当前格挡的伤害" : null) ??
            (text.StartsWith("M:repeat(1 + count(本场战斗你失去生命的次数))", StringComparison.Ordinal) ? "本场战斗中你每失去过一次生命，这张牌额外造成1次伤害" : null) ??
            (text.StartsWith("M:repeat(2)", StringComparison.Ordinal) ? "这张牌额外造成1次伤害" : null) ??
            (text.StartsWith("I:UpgradeThatCard", StringComparison.Ordinal) ? "升级那张攻击牌" : null) ??
            (text.StartsWith("I:打出抽牌堆顶牌，并强制消耗该牌", StringComparison.Ordinal) ? "打出抽牌堆顶部的牌并将其消耗" : null) ??
            (text.StartsWith("I:打出抽牌堆顶部X张牌", StringComparison.Ordinal) ? "打出抽牌堆顶部的X张牌" : null) ??
            (text.StartsWith("I:Transform(手牌全部可变形 Attack → 巨石)", StringComparison.Ordinal) ? "将手牌中的所有攻击牌变化为巨石" : null) ??
            (text.StartsWith("I:Create(random Attack, 手牌, 1, 本回合费用0)", StringComparison.Ordinal) ? "将一张随机攻击牌加入手牌。其本回合费用为0" : null) ??
            (text.StartsWith("I:连续抽牌，直到抽到非攻击牌", StringComparison.Ordinal) ? "抽牌，直到抽到一张非攻击牌" : null) ??
            (text.StartsWith("I:对一名随机敌人打出该牌", StringComparison.Ordinal) ? "对一名随机敌人打出这张牌" : null) ??
            (text.StartsWith("I:打出此牌", StringComparison.Ordinal) ? "则将其打出" : null) ??
            (text.StartsWith("I:额外打出一次该攻击", StringComparison.Ordinal) ? "将该攻击牌额外打出1次" : null) ??
            (text.StartsWith("I:随机消耗手牌1张Attack", StringComparison.Ordinal) ? "随机消耗手牌中的一张攻击牌" : null) ??
            (text.StartsWith("I:对每名敌人Apply(V,1)", StringComparison.Ordinal) ? "给予所有敌人1层易伤" : null) ??
            (text.StartsWith("I:Upgrade(select 1 card in hand)", StringComparison.Ordinal) ? "升级手牌中的一张牌" : null) ??
            (text.StartsWith("I:本回合获得S(", StringComparison.Ordinal) ? Regex.Replace(text[2..], @"本回合获得S\((\d+)\)", "本回合获得$1点力量") : null) ??
            (text.StartsWith("I:永久最大生命+3", StringComparison.Ordinal) ? "永久获得3点最大生命" : null) ??
            (text.StartsWith("I:本场战斗中此卡的基础伤害+5", StringComparison.Ordinal) ? "在本场战斗中，此卡的基础伤害增加5点" : null) ??
            (text.StartsWith("I:将该攻击牌的当前伤害永久加到此卡基础伤害", StringComparison.Ordinal) ? "将它的伤害添加给这张牌" : null) ??
            (text == "I:PreventDrawThisTurn" ? "本回合不能再抽牌" : null) ??
            (text == "I:FreeHandThisTurn" ? "你手牌中的所有牌在本回合免费打出" : null) ??
            Match(@"^I:ReplayNextSkills\((\d+)\)", "在本回合，你打出的下{0}张技能牌会被额外打出一次") ??
            (text == "I:DiscardHandDrawSame" ? "丢弃所有手牌，然后抽相同数量的牌" : null) ??
            Match(@"^I:DrawAndBlockIfSkill\((\d+),(\d+)\)", "抽{0}张牌。如果抽到的是技能牌，则获得{1}点格挡") ??
            Match(@"^I:DrawWithRetain\((\d+)\)", "抽{0}张牌。这些牌在本回合获得保留") ??
            (text == "I:GrantSlyToHandSkillThisTurn" ? "在本回合给手牌中的一张技能牌添加奇巧" : null) ??
            (text == "I:PlayExhaustedShivsAtTarget" ? "将消耗牌堆中的所有小刀对该敌人打出" : null) ??
            Match(@"^I:CopySelectedCardNextTurn\((\d+)\)", "选择一张手牌。在下个回合将它的{0}张复制加入手牌") ??
            (text == "I:TriggerPoisonNow" ? "立即触发所有敌人的中毒" : null) ??
            (text == "I:NextSkillCostsZero" ? "你的下一张技能牌耗能变为0" : null) ??
            (text == "I:DoubleAttackDamageNextTurn" ? "在下个回合，你所有的攻击伤害翻倍" : null) ??
            (text == "I:DoubleBlockThisTurn" ? "本回合你获得的格挡值翻倍" : null) ??
            (text == "I:AddCardReward" ? "额外获得一次卡牌奖励" : null) ??
            Match(@"^I:ReduceThisCardCostCombat\((\d+)\)", "本场战斗中，这张牌的耗能减少{0}") ??
            (text.StartsWith("I:", StringComparison.Ordinal) ? PolishTerms(text[2..]) : null) ??
            (text.StartsWith("A:turnStart", StringComparison.Ordinal) ? "在你的回合开始时" : null) ??
            (text.StartsWith("A:turnEnd", StringComparison.Ordinal) ? "在你的回合结束时" : null) ??
            (text.StartsWith("A:rule(你的格挡在回合开始时不移除)", StringComparison.Ordinal) ? "格挡不再在你的回合开始时消失" : null) ??
            (text.StartsWith("A:rule(你的技能费用=0)", StringComparison.Ordinal) ? "技能牌的耗能变为0" : null) ??
            (text.StartsWith("A:rule(有V的敌人额外受到25%伤害)", StringComparison.Ordinal) ? "拥有易伤的敌人受到的伤害增加25%" : null) ??
            (text.StartsWith("A:rule(每回合第一次从卡牌获得的格挡翻倍)", StringComparison.Ordinal) ? "每回合第一次通过卡牌获得的格挡翻倍" : null) ??
            Match(@"^A:rulePoisonExtraTriggers\((\d+)\)", "中毒会额外触发{0}次") ??
            Match(@"^A:ruleShivBonusDamage\((\d+)\)", "小刀额外造成{0}点伤害") ??
            Match(@"^A:ruleUnblockedAttackPoison\((\d+)\)", "每有一次攻击造成未被格挡的伤害，就给予{0}层中毒") ??
            (text == "A:ruleShivsHitAll" ? "小刀会攻击所有敌人" : null) ??
            (text == "A:rulePlayedSkillsGainSly" ? "当你打出技能牌时，该牌获得奇巧" : null) ??
            Match(@"^A:ruleShivsRetainAndFirstBonus\((\d+)\)", "小刀获得保留。每回合打出的第一张小刀额外造成{0}点伤害") ??
            (text == "A:ruleShivsRetain" ? "小刀获得保留" : null) ??
            Match(@"^A:ruleFirstShivBonusDamage\((\d+)\)", "每回合打出的第一张小刀额外造成{0}点伤害") ??
            Match(@"^A:ruleWeakEnemiesTakeMoreAttackDamage\((\d+)\)", "处于虚弱状态的敌人受到的攻击伤害增加{0}%") ??
            (text == "A:ruleWeakEnemiesTakeMoreAttackDamage" ? "处于虚弱状态的敌人受到的攻击伤害增加50%" : null) ??
            (text == "A:ruleRetainHand" ? "在你的回合结束时，不再丢弃你的手牌" : null) ??
            (text == "A:whenCardPlayed" ? "每当你打出一张牌时" : null) ??
            (text == "A:whenCardDrawnDuringTurn" ? "每当你在回合进行中抽到一张牌时" : null) ??
            (text.StartsWith("A:when(打出 Skill)", StringComparison.Ordinal) ? "每当你打出一张技能牌时" : null) ??
            (text.StartsWith("A:when(你的回合内失去生命)", StringComparison.Ordinal) ? "每当你在回合内失去生命时" : null) ??
            (text.StartsWith("A:when(任意牌被消耗)", StringComparison.Ordinal) ? "每当有一张牌被消耗时" : null) ??
            (text.StartsWith("A:when(每回合第3张 Attack 被打出)", StringComparison.Ordinal) ? "每回合中，当你打出第3张攻击牌时" : null) ??
            (text.StartsWith("A:when(抽到名字中有“打击”的牌)", StringComparison.Ordinal) ? "每当你抽到名字中有“打击”的牌时" : null) ??
            (text.StartsWith("A:when(你施加V)", StringComparison.Ordinal) ? "每当你施加易伤时" : null) ??
            (text.StartsWith("C:if(此伤害斩杀)", StringComparison.Ordinal) ? "斩杀时" : null) ??
            (text.StartsWith("C:if(count(消耗牌堆) >= 3)", StringComparison.Ordinal) ? "若消耗牌堆中至少有3张牌" : null) ??
            (text.StartsWith("C:if(本回合曾消耗牌)", StringComparison.Ordinal) ? "若本回合曾消耗过牌" : null) ??
            (text.StartsWith("C:if(你本回合失去过生命)", StringComparison.Ordinal) ? "若你本回合失去过生命" : null) ??
            (text.StartsWith("C:if(target has V)", StringComparison.Ordinal) ? "若该敌人拥有易伤" : null) ??
            (text == "C:ifLastDrawnSkill" ? "如果抽到的是技能牌" : null) ??
            (text.StartsWith("C:untilTurnEnd(每当你打出 Attack)", StringComparison.Ordinal) ? "本回合每当你打出一张攻击牌时" : null) ??
            (text.StartsWith("C:untilTurnEnd(forEach(你受到攻击))", StringComparison.Ordinal) ? "本回合每当你受到一次攻击时" : null) ??
            (text.StartsWith("C:untilTurnEnd(if attacker has V", StringComparison.Ordinal) ? "在本回合中，有易伤状态的敌人对你造成的伤害降低50%" : null) ??
            (text.StartsWith("C:forEach(被此效果消耗的牌)", StringComparison.Ordinal) ? "每消耗一张牌" : null) ??
            (text.StartsWith("C:forEach(手牌全部非Attack)", StringComparison.Ordinal) ? "每消耗一张手牌中的非攻击牌时" : null) ??
            (text.StartsWith("C:after(回合结束且card在消耗堆)", StringComparison.Ordinal) ? "在你的回合结束时，如果这张牌在你的消耗牌堆中" : null) ??
            (text.StartsWith("C:after(exhausted(card))", StringComparison.Ordinal) ? "这张牌被消耗时" : null) ??
            Match(@"^C:grantNextAttacksThisTurn\((\d+)\)", "在这个回合，你打出的下{0}张攻击牌获得效果：") ??
            (text.StartsWith("C:grantNextAttack", StringComparison.Ordinal) ? "你打出的下一张攻击牌获得效果：" : null) ??
            (text.StartsWith("C:untilTurnEnd(for next 1 played Attack", StringComparison.Ordinal) ? "当你打出下一张攻击牌时" : null) ??
            (text.StartsWith("C:for next 1 played Attack", StringComparison.Ordinal) ? "当你打出下一张攻击牌时" : null) ??
            (text.StartsWith("C:whileInCombat(本回合每打出过1张Attack", StringComparison.Ordinal) ? "你在本回合中每打出过一张攻击牌，其耗能减少1" : null) ??
            (text == "C:ifTargetPoisoned" ? "若该敌人拥有中毒" : null) ??
            (text == "C:untilTurnEndCardDrawn" ? "本回合每当你抽到一张牌时" : null) ??
            (text == "C:playableIfDrawPileEmpty" ? "只有当抽牌堆中没有牌时" : null) ??
            (text == "C:whileInCombatSkillCostReduction" ? "你在本回合中每打出过一张技能牌，其耗能减少1" : null) ??
            (text == "C:forEachDiscarded" ? "每丢弃一张牌" : null) ??
            (text == "C:ifFatal" ? "斩杀时" : null) ??
            (text == "T:RemoveBlockAndArtifact" ? "去除该敌人的所有格挡和人工制品" : null) ??
            (text == "M:RepeatAreaOnKill" ? "每有一名敌人被击杀，就重复此效果" : null) ??
            (text == "M:RepeatPerAttackThisTurn" ? "本回合每打出过一张攻击牌，就造成一次伤害" : null) ??
            (text == "M:RepeatPerSkillInHand" ? "手牌中每有一张技能牌，就造成一次伤害" : null) ??
            Match(@"^M:DamagePerDiscardThisTurn\((\d+)\)", "本回合每丢弃过一张牌，这张牌额外造成{0}点伤害") ??
            Match(@"^M:DamagePerCardDrawnCombat\((\d+)\)", "本场战斗每抽过一张牌，这张牌额外造成{0}点伤害") ??
            Match(@"^M:DamageMinusPerCardInHand\((\d+)\)", "手牌中每有一张其他牌，这张牌的伤害降低{0}点") ??
            (text.StartsWith("C:", StringComparison.Ordinal) ? $"当满足{PolishTerms(text[2..])}时" : null) ??
            (text.StartsWith("A:when(", StringComparison.Ordinal) ? $"每当{PolishTerms(text[7..].TrimEnd(')'))}时" : null) ??
            (text.StartsWith("N:Move(选择1张, 弃牌堆→抽牌堆顶)", StringComparison.Ordinal) ? "将弃牌堆中的一张牌放到抽牌堆顶部" : null) ??
            (text.StartsWith("N:Move(random Attack, 弃牌堆→手牌)", StringComparison.Ordinal) ? "将弃牌堆中的一张随机攻击牌放入手牌" : null) ??
            (text.StartsWith("N:Create(random card from own pool, 手牌, 1)", StringComparison.Ordinal) ? "将一张当前角色的随机牌加入手牌" : null) ??
            (text.StartsWith("N:Exhaust(该技能)", StringComparison.Ordinal) ? "消耗那张技能牌" : null) ??
            (text.StartsWith("T:Apply(enemy S,1)", StringComparison.Ordinal) ? "使该敌人获得1点力量" : null) ??
            (text.StartsWith("T:Apply(本回合敌方S-10)", StringComparison.Ordinal) ? "使该敌人在本回合失去10点力量" : null) ??
            (text.StartsWith("T:Apply(V, count(target.V after damage))", StringComparison.Ordinal) ? "将该敌人身上的易伤层数翻倍" : null) ??
            (text.StartsWith("N:Self(S, count(target.V) after applying)", StringComparison.Ordinal) ? "目标敌人身上每有一层易伤，就获得1点力量" : null) ??
            (text.StartsWith("N:反伤攻击者D(4)", StringComparison.Ordinal) ? "本回合每当你受到一次攻击，对攻击者造成4点伤害" : null) ??
            PolishTerms(text.Replace("A:rule", "规则").Replace("N:", "").Replace("T:", "").Replace("M:", ""));

        return Finish(rendered);
    }

    private static string HitSuffix(string hits) => hits switch
    {
        "1" => string.Empty,
        "本次支付的X" => "X次",
        _ => $"{hits}次"
    ...
    private static string PolishTerms(string text) => text
        .Replace("Attack", "攻击牌")
        .Replace("Skill", "技能牌")
        .Replace("Power", "能力牌")
        .Replace("count(target.V)", "目标的易伤层数")
        .Replace("target.V", "目标的易伤层数")
        .Replace("Self(S", "获得力量(")
        .Replace("Create(random Attack", "将一张随机攻击牌加入手牌")
        .Replace("Upgrade(select 1 card in hand)", "升级手牌中的一张牌");

    private static string Finish(string text) => text.EndsWith('。') || text.EndsWith('，') || text.EndsWith('：') ? text : text + "。";
}

/// <summary>Loads 86 unupgraded single-player component sequences from the reviewed Silent decomposition.</summary>
public sealed class SilentComponentCatalog : IComponentCatalog
{
    private static readonly Lazy<SilentComponentCatalog> LazyInstance = new(Load);
    public static SilentComponentCatalog Instance => LazyInstance.Value;
    public GeneratedCharacter Character => GeneratedCharacter.Silent;
    public IReadOnlyList<IroncladCardRecipe> Recipes { get; }
    public IReadOnlyList<ComponentAtom> Atoms { get; }
    public IReadOnlyList<int> ComponentCounts { get; }
    public IReadOnlySet<string> AtomKeys { get; }
    ...
    private SilentComponentCatalog(IReadOnlyList<IroncladCardRecipe> recipes)
    {
        Recipes = recipes;
        var allAtoms = recipes.SelectMany(recipe => recipe.Atoms).ToArray();
        Atoms = allAtoms.DistinctBy(atom => atom.SchemaKey).ToArray();
    ...
    private static SilentComponentCatalog Load()
    {
        var assembly = Assembly.GetExecutingAssembly();
        var resource = assembly.GetManifestResourceNames().Single(name => name.EndsWith("silent_unit_operations.md", StringComparison.Ordinal));
        using var stream = assembly.GetManifestResourceStream(resource) ?? throw new InvalidOperationException("缺少嵌入的猎手组件目录。");
        using var reader = new StreamReader(stream);
        var recipes = reader.ReadToEnd().Split('\n')
            .Select(IroncladComponentCatalog.ParseRow)
            .Where(recipe => recipe is not null)
            .Cast<IroncladCardRecipe>()
            .ToArray();
        if (recipes.Length != 86)
            throw new InvalidOperationException($"猎手组件目录应为 86 张卡，实际读取到 {recipes.Length} 张。");
        return new SilentComponentCatalog(CatalogRuntimeSpecRegistry.Attach(GeneratedCharacter.Silent,
            recipes));
    }
}
#endif
