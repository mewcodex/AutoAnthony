...
public static class CardTextStyle
{
    ...
    public static string Chinese(GeneratorOperation operation, string effectiveText)
    {
        ...
        var text = operation.Template switch
        {
            ...
            _ when runtimeSpec.Trigger is { Kind: "next_attack_played", Lifetime: "this_turn" }
            ...
            _ when runtimeSpec.Trigger?.Kind == "attack_played"
                || operation.RuntimeSpec is null
            ...
            _ when runtimeSpec.Trigger?.Kind == "attack_received"
                || operation.RuntimeSpec is null
            ...
            _ when runtimeSpec.Trigger?.Kind == "card_played"
                || operation.RuntimeSpec is null
            ...
            "I:Create" => Regex.Replace(effectiveText,
                @"^将一张((?:升级过的)?随机攻击牌)加入(?:你的)?手牌。(?:其本回合费用为0|该牌在本回合费用为0|该牌在本回合内可以免费打出)。?$",
                "将一张$1加入你的手牌。该牌在本回合内可以免费打出。"),
