...
def ref(component: str, /, trigger: int = -1,
        text: tuple[str, str] | None = None, **arguments: Any) -> dict[str, Any]:
    result = {
    ...
    if text is not None:
        result["Text"] = {"zhHans": text[0], "en": text[1]}
    return result
...
def special_components(card: str) -> list[dict[str, Any]]:
    ...
    mapping: dict[str, list[dict[str, Any]]] = {
        ...
        "ByrdSwoop": [ref("deal.damage", amount=v("Damage"), target="SelectedEnemy", hits=1,
                          text=("造成{Damage:diff()}点伤害。", "Deal {Damage:diff()} damage."))],
        "Caltrops": [ref("trigger.when_attacked",
                         text=("每当你被攻击时，", "Whenever you are attacked, ")),
                     ref("deal.damage_to_attacker", trigger=0, amount=v("ThornsPower"),
                         text=("对攻击者造成{ThornsPower:diff()}点伤害。",
                               "deal {ThornsPower:diff()} damage back."))],
        "Clash": [ref("play_condition.hand_all_type", cardType="Attack",
                      text=("只有在[gold]手牌[/gold]中每一张牌都是攻击牌时才能被打出。",
        ...
        "Distraction": [ref("create.random_card", cardType="Skill", destination="Hand", count=1,
                            freeDuration="Turn",
                            text=("将一张随机技能牌添加到你的[gold]手牌[/gold]中。该牌在本回合内可以免费打出。",
                                  "Add a random Skill into your [gold]Hand[/gold]. It is free to play this turn."))],
        ...
        "DualWield": [ref("select.hand_card", cardTypes=["Attack", "Power"], count=1,
                          text=("选择一张攻击牌或能力牌。", "Choose an Attack or Power card. ")),
                      ref("create.copy_of_selected", trigger=0, destination="Hand", count=v("Cards"),
                          text=("将该牌的{IfUpgraded:show:{Cards}张复制品|一张复制品}加入你的[gold]手牌[/gold]。",
                                "Add {IfUpgraded:show:{Cards} copies|a copy} of that card into your [gold]Hand[/gold]."))],
        "Enlightenment": [ref("set.hand_cost", amount=1, duration="TurnOrCombatOnUpgrade",
                              text=("在这{IfUpgraded:show:场战斗|个回合}，你当前[gold]手牌[/gold]中所有牌的耗能降低至1。",
        ...
        "Entrench": [ref("multiply.current_block", multiplier=2,
                         text=("将你当前的[gold]格挡[/gold]翻倍。", "Double your [gold]Block[/gold]."))],
        "Exterminate": [ref("deal.damage", amount=v("Damage"), target="AllEnemies", hits=v("Repeat"),
                            text=("对所有敌人造成{Damage:diff()}点伤害{Repeat:diff()}次。",
        ...
        "FeedingFrenzy": [ref("gain.power", power="Strength", amount=v("StrengthPower"), duration="Turn",
                              text=("在本回合内获得{StrengthPower:diff()}点[gold]力量[/gold]。",
        ...
        "HelloWorld": [ref("trigger.turn_start", text=("在你的回合开始时，", "At the start of your turn, ")),
                       ref("create.random_card", trigger=0, rarity="Common", destination="Hand", count=1,
                           text=("将一张随机普通牌加入你的[gold]手牌[/gold]。",
                                 "add a random Common card into your [gold]Hand[/gold]."))],
        ...
        "Metamorphosis": [ref("create.random_card", cardType="Attack", destination="Draw", count=v("Cards"),
                              freeDuration="Combat",
                              text=("在你的[gold]抽牌堆[/gold]中加入{Cards:diff()}张随机攻击牌。它们在本场战斗中可以被免费打出。",
                                    "Add {Cards:diff()} random Attacks into your [gold]Draw Pile[/gold]. They are free to play this combat."))],
        ...
        "Rebound": [ref("deal.damage", amount=v("Damage"), target="SelectedEnemy", hits=1,
                    ...
                    ref("next_played_card.move", destination="DrawTop", duration="Turn",
                        text=("将你在本回合打出的下一张牌放置到你的[gold]抽牌堆[/gold]顶部。",
        ...
        "RipAndTear": [ref("deal.damage", amount=v("Damage"), target="RandomEnemy", hits=2,
                           text=("随机对敌人造成{Damage:diff()}点伤害两次。",
        ...
        "Squash": [ref("deal.damage", amount=v("Damage"), target="SelectedEnemy", hits=1,
                   ...
                   ref("apply.power", power="Vulnerable", amount=v("VulnerablePower"), target="SelectedEnemy",
                       text=("给予{VulnerablePower:diff()}层[gold]易伤[/gold]。",
        ...
        "Stack": [ref("gain.block_scaled", base=v("CalculationBase"), scale="DiscardPileCount", multiplier=1,
                      text=("获得等量于与你当前[gold]弃牌堆[/gold]中牌数{IfUpgraded:show:+{CalculationBase}|}的[gold]格挡[/gold]值。{InCombat:\n（获得{CalculatedBlock:diff()}点[gold]格挡[/gold]）|}",
                            "Gain [gold]Block[/gold] equal to the number of cards in your [gold]Discard Pile[/gold]{IfUpgraded:show: +{CalculationBase}|}.{InCombat:\n(Gain {CalculatedBlock:diff()} [gold]Block[/gold])|}"))],
        "SweepingGaze": [ref("osty.deal_damage", amount=v("OstyDamage"), target="RandomEnemy")],
        "ToricToughness": [ref("gain.block", amount=v("Block"),
                               text=("获得{Block:diff()}点[gold]格挡[/gold]。",
                                     "Gain {Block:diff()} [gold]Block[/gold].")),
                           ref("trigger.next_turn_starts", duration=v("Turns"),
                               text=("在接下来的{Turns:diff()}个回合开始时，",
                                     "At the start of the next {Turns:diff()} {Turns:plural:turn|turns}, ")),
...
MAD_SCIENCE = {
    "AttackSapping": ("Attack", "AnyEnemy", [
        ...
        ref("apply.power", power="Weak", amount=arg_var("SappingWeak"), target="SelectedEnemy",
            text=("给予{SappingWeak:diff()}层[gold]虚弱[/gold]。",
        ...
        ref("apply.power", power="Vulnerable", amount=arg_var("SappingVulnerable"), target="SelectedEnemy",
            text=("给予{SappingVulnerable:diff()}层[gold]易伤[/gold]。",
                  "Apply {SappingVulnerable:diff()} [gold]Vulnerable[/gold]."))]),
    "AttackViolence": ("Attack", "AnyEnemy", [
        ref("deal.damage", amount=arg_var("Damage"), target="SelectedEnemy", hits=arg_var("ViolenceHits"),
            text=("造成{Damage:diff()}点伤害{ViolenceHits:diff()}次。",
                  "Deal {Damage:diff()} damage {ViolenceHits:diff()} times."))]),
    "AttackChoking": ("Attack", "AnyEnemy", [
        ref("deal.damage", amount=arg_var("Damage"), target="SelectedEnemy", hits=1,
            text=("造成{Damage:diff()}点伤害。", "Deal {Damage:diff()} damage.")),
        ref("trigger.after_card_played", duration="Turn", excludesSource=False,
            text=("本回合，你每打出一张牌，", "Whenever you play a card this turn, ")),
        ref("lose.enemy_hp", trigger=1, amount=arg_var("ChokingDamage"), target="SelectedEnemy",
            text=("该敌人失去{ChokingDamage:diff()}点生命。",
                  "the enemy loses {ChokingDamage:diff()} HP."))]),
    "SkillEnergized": ("Skill", "Self", [
        ref("gain.block", amount=arg_var("Block"),
            text=("获得{Block:diff()}点[gold]格挡[/gold]。", "Gain {Block:diff()} [gold]Block[/gold].")),
        ref("gain.energy", amount=arg_var("EnergizedEnergy"),
            text=("获得{EnergizedEnergy:energyIcons()}。", "Gain {EnergizedEnergy:energyIcons()}."))]),
    "SkillWisdom": ("Skill", "Self", [
        ref("gain.block", amount=arg_var("Block"),
            text=("获得{Block:diff()}点[gold]格挡[/gold]。", "Gain {Block:diff()} [gold]Block[/gold].")),
        ref("draw.cards", amount=arg_var("WisdomCards"),
            text=("抽{WisdomCards:diff()}张牌。", "Draw {WisdomCards:diff()} cards."))]),
    "SkillChaos": ("Skill", "Self", [
        ref("gain.block", amount=arg_var("Block"),
            text=("获得{Block:diff()}点[gold]格挡[/gold]。", "Gain {Block:diff()} [gold]Block[/gold].")),
        ref("create.random_card", count=1, destination="Hand", freeDuration="Turn",
            text=("将一张随机牌放入你的[gold]手牌[/gold]。该牌在本回合可以免费打出。",
                  "Add a random card into your [gold]Hand[/gold]. It is free to play this turn."))]),
    "PowerExpertise": ("Power", "Self", [
        ref("gain.power", power="Strength", amount=arg_var("ExpertiseStrength"),
            text=("获得{ExpertiseStrength:diff()}点[gold]力量[/gold]。",
                  "Gain {ExpertiseStrength:diff()} [gold]Strength[/gold].")),
        ref("gain.power", power="Dexterity", amount=arg_var("ExpertiseDexterity"),
            text=("获得{ExpertiseDexterity:diff()}点[gold]敏捷[/gold]。",
                  "Gain {ExpertiseDexterity:diff()} [gold]Dexterity[/gold]."))]),
    "PowerCurious": ("Power", "Self", [
        ref("card_type.cost_reduction", cardType="Power", amount=arg_var("CuriousReduction"),
            duration="Combat",
            text=("能力牌的耗能减少{CuriousReduction:diff()}{energyPrefix:energyIcons(1)}。",
                  "Powers cost {CuriousReduction:diff()} {energyPrefix:energyIcons(1)} less."))]),
    "PowerImprovement": ("Power", "Self", [
        ref("trigger.combat_end", text=("在战斗结束时，", "At the end of combat, ")),
        ref("upgrade.random_deck_card", trigger=0, count=1,
            text=("[gold]升级[/gold]你牌组中的一张随机牌。",
                  "[gold]Upgrade[/gold] a random card in your deck."))]),
}
